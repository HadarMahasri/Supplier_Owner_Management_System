...
# כך נשמר רצף עיצובי/חווייתי בין שני הדפים.
...
class OwnerLinksPage(QWidget):
    ...
    def _add_supplier_card(self, parent_layout, user_dict):
        ...
        # פתרון חכם: אם הטקסט מכיל אנגלית, ניישר לימין. אחרת נשאיר טבעי
        if any(ord(char) < 128 and char.isalpha() for char in company_name):
    ...
    def _add_pending_card(self, parent_layout, link):
        sup = link.get("owner", {})  # כמו בדף הספק – OwnerMini משמש גם כאן כספק
        card = QWidget()
        ...
        # תאריך הבקשה בצד השמאלי (אם קיים)
        requested_at = link.get("requested_at") or link.get("created_at")
        if requested_at:
            if isinstance(requested_at, str):
                # אם זה string, נתנסה לחלץ תאריך ושעה
            ...
            date_container.setSpacing(1)
            
            date_title = QLabel("בקשה מתאריך:")
            date_title.setStyleSheet("font-size: 9px; color: #9ca3af; font-weight: 600;")
            date_title.setAlignment(Qt.AlignCenter)
            
            date_value = QLabel(date_text)
            date_value.setStyleSheet("font-size: 11px; color: #6b7280; font-weight: 500;")
            date_value.setAlignment(Qt.AlignCenter)
            
            date_container.addWidget(date_title)
            date_container.addWidget(date_value)
            
            # הוספת שעה אם קיימת
            if time_text:
                time_value = QLabel(time_text)
                time_value.setStyleSheet("font-size: 10px; color: #9ca3af;")
                time_value.setAlignment(Qt.AlignCenter)
                date_container.addWidget(time_value)
            
            date_widget = QWidget()
            date_widget.setLayout(date_container)
            date_widget.setFixedWidth(90)
    ...
    def _add_active_card(self, parent_layout, link):
        ...
        # תאריך החיבור בצד השמאלי הקיצוני (אם קיים)
    ...
    def _place_order(self, supplier_id):
        dlg = OrderCreateDialog(self.owner_id, supplier_id, self)
        dlg.exec()
